<script lang="ts">
  import { setPaymentContado, setPaymentCredito } from '$lib/store/actions/payment-terms.actions'
  import type { Cuota } from '$lib/store/actions/payment-terms.actions'
  import { documentStore } from '$lib/store/document.store'
  import { getPaymentMethods } from './payment-terms.component'
  ...
  const availableMethods = $derived(
    getPaymentMethods(currentDocumentType)
  )

  $effect(() => {
    if (!availableMethods.includes(metodo)) {
      setMetodo('Contado')
    }
  })
  ...
  function setMetodo(m: MetodoPago) {
    ...
    if (m === 'Contado') {
      ...
      setPaymentCredito(total, cuotas.map(c => ({ ...c })))
    }
  }
  ...
  function updateCuota(id: number, field: keyof Omit<Cuota, 'id'>, value: string) {
    cuotas = cuotas.map(c => c.id === id ? { ...c, [field]: value } : c)
    syncCuotas()
  }

  const inputClass =
    'block w-full rounded-xl border border-[color:color-mix(in_oklab,var(--form-color-3)_30%,transparent)] bg-[var(--form-field-bg)] px-3 py-2 text-sm text-[var(--form-text-color)] outline-none transition focus:border-[var(--form-color-3)]'
</script>
...
<div class="overflow-hidden rounded-[1.15rem] border border-[color:color-mix(in_oklab,var(--form-color-3)_22%,transparent)] bg-[var(--form-panel-bg)]">
  ...
  <div class="flex items-center justify-between gap-4 border-b border-[color:color-mix(in_oklab,var(--form-color-3)_16%,transparent)] px-5 py-3">
    <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--form-text-soft)]">
      Método de pago
    </p>
    <div class="flex items-center gap-1 rounded-full border border-[color:color-mix(in_oklab,var(--form-color-3)_28%,transparent)] bg-[var(--form-field-bg)] p-1">
      {#each availableMethods as m}
        <button
          class="rounded-full px-3 py-1 text-[12px] font-medium transition {metodo === m ? 'bg-[var(--form-color-3)] text-white shadow-sm' : 'text-[var(--form-text-soft)] hover:text-[var(--form-text-color)]'}"
          onclick={() => setMetodo(m)}
          type="button"
        >{m}</button>
      {/each}
  ...
  {#if metodo === 'Contado'}
    <div class="flex items-center gap-3 px-5 py-4">
      <div class="flex size-8 shrink-0 items-center justify-center rounded-full bg-[color:color-mix(in_oklab,var(--form-color-3)_12%,transparent)] text-[var(--form-text-soft)]">
  ...
  {:else}
    <div class="px-5 py-4 space-y-3">
        ...
        <div class="grid grid-cols-[1fr_1fr_28px] items-center gap-2">
          <div class="relative">
            <div class="mb-1 flex items-center gap-1.5">
              <span class="flex size-4 items-center justify-center rounded-full bg-[color:color-mix(in_oklab,var(--form-color-3)_14%,transparent)] text-[10px] font-semibold tabular-nums text-[var(--form-text-soft)]">{i + 1}</span>
              <span class="text-[11px] text-[var(--form-text-soft)]">Monto</span>
            </div>
            <div class="relative">
              <input
                class="{inputClass} ps-7 tabular-nums"
                inputmode="decimal"
                placeholder="0.00"
                type="text"
                value={cuota.monto}
          ...
          <button
            ...
            class="mt-5 inline-flex size-7 items-center justify-center rounded-full border border-[color:color-mix(in_oklab,var(--form-color-3)_22%,transparent)] bg-transparent text-[var(--form-text-soft)] transition hover:border-red-400/40 hover:text-red-500 disabled:pointer-events-none disabled:opacity-25"
      ...
      <button
        ...
        type="button"
      >
        <svg class="size-3 shrink-0" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" viewBox="0 0 24 24">
      ...
      {#if cuotas.length > 0}
        ...
        {@const diff = Math.abs(sumaCuotas - total)}
        {#if diff > 0.01}
