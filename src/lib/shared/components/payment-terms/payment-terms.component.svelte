<script lang="ts">
  ...
  onMount(() => {
    hidratarDesdeDocumento();
    const unsubscribe = documentLoaded.subscribe((event) => {
      if (event) hidratarDesdeDocumento();
    });
    return unsubscribe;
  });
...
<div class="overflow-hidden rounded-[1.15rem] border border-[color:color-mix(in_oklab,var(--form-color-3)_22%,transparent)] bg-[var(--form-panel-bg)]">
  <div class="flex items-center justify-between gap-4 border-b border-[color:color-mix(in_oklab,var(--form-color-3)_16%,transparent)] px-5 py-3">
    ...
    <div class="flex items-center gap-1 rounded-full border border-[color:color-mix(in_oklab,var(--form-color-3)_28%,transparent)] bg-[var(--form-field-bg)] p-1">
      {#each ["Contado", "Credito"] as MetodoPago[] as m}
        <button
          class="rounded-full px-3 py-1 text-[12px] font-medium transition {metodo === m
  ...
  {:else}
    <div class="px-5 py-4 space-y-3">
      {#each cuotas as cuota, i (cuota.id)}
        ...
        <div class="grid grid-cols-[1fr_1fr_28px] items-start gap-2">
          <div>
            ...
            <div class="relative">
              <input
                class="{err?.monto ? inputErrorClass : inputClass} ps-7 tabular-nums"
                inputmode="decimal"
                placeholder="0.00"
                type="text"
                value={cuota.monto}
      ...
      {#if montoExcede}
        <div class="flex items-center gap-2 rounded-xl border border-red-400/30 bg-red-50/60 px-3 py-2 text-[12px] text-red-700 dark:bg-red-900/20 dark:text-red-400">
          <svg class="size-3.5 shrink-0" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line x1="12" x2="12.01" y1="16" y2="16" />
          </svg>
