<script lang="ts">
    ...
    let valorFocused = $state(false); // esto es para mostrar el valor formateado sin tocar el store el cual va a tener todos los decimales
    let precioFocused = $state(false); // esto es para mostrar el precio formateado sin tocar el store el cual va a tener todos los decimales

    let editorItem = $state(createEditableItem());
    let lastEdited = $state<"valor" | "precio" | null>(null);
    const valorDisplay = $derived(
        valorFocused || editorItem.valorUnitario === ""
        ? editorItem.valorUnitario
        : parseFloat(editorItem.valorUnitario || "0").toFixed(2),
    );

    const precioDisplay = $derived(
        precioFocused || editorItem.precioUnitario === ""
        ? editorItem.precioUnitario
        : parseFloat(editorItem.precioUnitario || "0").toFixed(2),
    );
    $effect(() => {
        editorItem = isOpen
    ...
    //Validar si el tipo de comprobante es Nota de crédito (07) y el motivo es Anulación de la operación (03) para permitir precio unitario en 0
